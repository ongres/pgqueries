...
select  sub.table,sub.filter,(select text as query from pg_catalog.stl_querytext sq where query = sub.query) as querytext  from 
(select 
...
group by 1,2 order by 1, 3 desc , 4 desc) sub ;